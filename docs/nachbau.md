# 5RC Gitzmod — Nachbau-Anleitung

Vollständige Anleitung inklusive **gesamtem Quellcode**, um diese Flipper-Zero-App von Grund
auf nachzubauen: eine Infrarot-Fernbedienung für Samsung-Klimageräte mit Power, Betriebsart,
Temperatur, Lüfterstufe, vertikalem und horizontalem Swing sowie WindFree.

Die Bedienung ist in [`bedienung.md`](bedienung.md) beschrieben, Screenshots liegen in
[`screenshots/`](screenshots/).

Basis ist [dappermint/samsung-ac-remote-flipper-app](https://github.com/dappermint/samsung-ac-remote-flipper-app)
(GPL), dessen Panel-Code wiederum von
[flipperzero-midea-ac-remote](https://github.com/xakep666/flipperzero-midea-ac-remote) stammt.
Neu hinzugekommen sind WindFree, der horizontale Swing, die Zustandsanzeige der unteren
Buttons und die Überschrift.
...
2. [Das Samsung-Protokoll](#2-das-samsung-protokoll)
3. [Verifikation gegen echte Mitschnitte](#3-verifikation-gegen-echte-mitschnitte)
4. [Build-Umgebung](#4-build-umgebung)
5. [Projektstruktur](#5-projektstruktur)
6. [Der Protokoll-Code](#6-der-protokoll-code)
7. [Die App](#7-die-app)
8. [Das Panel-Widget](#8-das-panel-widget)
9. [Icons und Assets](#9-icons-und-assets)
10. [Bauen und aufspielen](#10-bauen-und-aufspielen)
11. [Screenshots erzeugen](#11-screenshots-erzeugen)
12. [Fallstricke](#12-fallstricke)
...
- Flipper Zero (Target `f7`, Hardware-Version 12 in diesem Fall)
- Ein Samsung-Klimagerät in IR-Reichweite

**Firmware auf dem Gerät**

Diese App wurde gegen die **offizielle Firmware 1.4.3, API 87.1** gebaut. Die API-Version
muss zwischen Build und Gerät zusammenpassen, sonst verweigert der Loader den Start.
Prüfen lässt sich das über die serielle CLI:

```
device_info
```

Relevant sind `firmware_api_major` / `firmware_api_minor` und `firmware_origin_fork`.

**Auf dem Rechner**

- Python 3 (hier 3.13)
- ARM-Toolchain `arm-none-eabi-gcc` **12.3.1** — die Version, mit der die Flipper-SDK
  gebaut wird
- SCons plus einige Python-Pakete, siehe [Build-Umgebung](#4-build-umgebung)

---

## 2. Das Samsung-Protokoll

### Zustandsbehaftet, nicht befehlsbasiert

Samsung-Klimageräte kennen **keine Einzelbefehle** wie „Temperatur +1". Jeder Tastendruck
der Originalfernbedienung überträgt den **kompletten Gerätezustand** in einem Frame. Die App
muss also bei jedem Knopfdruck alles mitsenden, was gerade eingestellt ist.

Das ist auch der Grund, warum Brute-Force hier nicht funktioniert: ein zufälliges Bitmuster
ist praktisch nie ein gültiger Frame mit korrekten Prüfsummen.

### Rahmenformat

Das Standard-Frame ist **14 Byte**, aufgeteilt in **zwei Sektionen zu je 7 Byte**. Jede
Sektion trägt ihre **eigene Prüfsumme**.

```
Trägerfrequenz   38 kHz, 50 % Tastverhältnis

Header           Mark  690 µs   Space 17844 µs
je Sektion       Mark 3086 µs   Space  8864 µs
                 56 Bit, LSB zuerst
                   Bit-Mark      586 µs
                   Eins-Space   1432 µs
                   Null-Space    436 µs
                 Footer Mark 586 µs, Lücke 2886 µs
```

Macht 2 + 2 × (2 + 56×2 + 2) = **234 Timing-Werte** pro Übertragung. Das Limit der
Flipper-Infrarot-Bibliothek liegt bei `MAX_TIMINGS_AMOUNT` = 1024, passt also locker.

### Feldbelegung

Byte-Offsets beziehen sich auf das 14-Byte-Frame:

| Byte | Bits | Feld | Werte |
|---|---|---|---|
| 1 | 4–7 | Prüfsumme 1, unteres Nibble | |
| 2 | 0–3 | Prüfsumme 1, oberes Nibble | |
| 6 | 4–5 | Power 1 | `0b11` ein, `0b00` aus |
| 8 | 4–7 | Prüfsumme 2, unteres Nibble | |
| 9 | 0–3 | Prüfsumme 2, oberes Nibble | |
| 9 | 4–6 | Swing | `0b111` aus, `0b010` vertikal, `0b011` horizontal, `0b100` beides |
| 10 | 1–3 | FanSpecial | `0b000` aus, `0b011` Powerful, `0b101` **WindFree**, `0b111` Econo |
| 10 | 4 | Display | |
| 11 | 4–7 | Temperatur | Sollwert − 16, also `0`…`14` für 16–30 °C |
| 12 | 1–3 | Lüfter | `0` Auto, `2` niedrig, `4` mittel, `5` hoch |
| 12 | 4–6 | Betriebsart | `0` Auto, `1` Kühlen, `2` Entfeuchten, `3` Ventilator, `4` Heizen |
| 13 | 4–5 | Power 2 | wie Power 1 |

**Zwei Fallstricke**, die man beim Nachbau leicht übersieht:

- **Power steht doppelt im Frame** (Byte 6 und Byte 13). Beide müssen gesetzt werden.
- **Vertikaler und horizontaler Swing teilen sich ein einziges 3-Bit-Feld.** Wer nur eine
  Achse schreibt, löscht die andere. Die Funktion muss beide Achsen entgegennehmen.

**WindFree** akzeptiert das Gerät nur zusammen mit **Lüfter auf Auto** und **vertikalem Swing
aus** — die Referenzimplementierung prüft in `getBreeze()` genau diese Kombination. Die App
setzt das beim Einschalten mit und fällt bei Änderung von Lüfter oder V-Swing wieder heraus.

### Prüfsumme

Pro Sektion: Anzahl gesetzter Bits zählen, dabei die Prüfsummen-Nibbles selbst auslassen,
und das Ergebnis bitweise invertieren.

```
summe  = popcount(sektion[0])
       + popcount(sektion[1] & 0x0F)     // oberes Nibble ist die Prüfsumme
       + popcount((sektion[2] >> 4) & 0x0F)
       + popcount(sektion[3..6])
prüfsumme = summe XOR 0xFF
```

Die 8 Bit landen dann auf zwei Nibbles verteilt: unteres Nibble ins **obere** Nibble von
Byte 1 der Sektion, oberes Nibble ins **untere** Nibble von Byte 2.

### Quellen

Timings, Feldbelegung und Prüfsumme stammen aus der Reverse-Engineering-Arbeit in
[IRremoteESP8266](https://github.com/crankyoldgit/IRremoteESP8266), Datei `ir_Samsung.cpp`,
ursprünglich dokumentiert in
[Issue #1538](https://github.com/crankyoldgit/IRremoteESP8266/issues/1538) (Prüfsumme) und
[Issue #1062](https://github.com/crankyoldgit/IRremoteESP8266/issues/1062) (WindFree).

---

## 3. Verifikation gegen echte Mitschnitte

Statt darauf zu vertrauen, dass die Implementierung stimmt, lässt sie sich gegen echte, von
Original-Fernbedienungen mitgeschnittene Frames prüfen. Die Testsuite von IRremoteESP8266
enthält solche Mitschnitte samt dekodierter Bedeutung.

Das folgende Skript baut die Frames mit derselben Logik wie der C-Code und vergleicht sie
Byte für Byte:
...
# Mitschnitt aus IRremoteESP8266 Issue #505:
...
# Mitschnitt: "Power: On, Mode: Cool, Temp: 16C, Fan: Low, Swing(V): On"
assert build('cool', 16, 'low', True, False, False, True) == \
    [0x02,0x92,0x0F,0x00,0x00,0x00,0xF0, 0x01,0x02,0xAF,0x71,0x00,0x15,0xF0]

print("beide Frames stimmen bitgenau")
```

Beide Vergleiche gehen auf — inklusive beider Prüfsummen. Was die App sendet, ist exakt das,
was eine Originalfernbedienung sendet.
...
Für Flipper-Apps gibt es **uFBT**, das SDK und Toolchain selbst herunterlädt:

```bash
pip install --upgrade ufbt
cd <app-verzeichnis>
ufbt              # baut die .fap
ufbt launch       # baut, kopiert aufs Gerät und startet
```

Das ist alles, was man normalerweise braucht.

### Wenn `update.flipperzero.one` blockiert ist

In einem Firmennetz mit DNS-Filter (hier Cisco Umbrella) ist die Update-Domain gesperrt,
womit uFBT weder SDK noch Toolchain laden kann:
...
Der Ausweg besteht aus drei Teilen — alle Quellen liegen auf GitHub:

**a) Firmware-Quellbaum statt SDK-Paket.** Die offiziellen Release-Assets liegen nur auf der
gesperrten Domain, der Quellcode aber auf GitHub. Mit dem Baum lässt sich direkt bauen:
...
> Unter Windows unbedingt an einen **kurzen Pfad** klonen (z. B. `C:\ffw`) und
> `git config core.longpaths true` setzen. Sonst scheitert das mbedtls-Submodul mit
> *„Filename too long"*.

**b) ARM-Toolchain von xPack.** Versionsgleich mit der offiziellen (GCC 12.3.1):

```
https://github.com/xpack-dev-tools/arm-none-eabi-gcc-xpack/releases/tag/v12.3.1-1.2
```

Entpacken und so ablegen, dass `<toolchain>/bin/arm-none-eabi-gcc` existiert.

**c) Toolchain-Download überspringen.** `FBT_NOENV=1` sagt dem Build-System, dass die
Umgebung bereits steht, damit lädt es nichts nach:
...
Die App wird über einen Verzeichnis-Link in den Firmware-Baum eingehängt:

```powershell
New-Item -ItemType Junction -Path C:\ffw\applications_user\samsung_ac_remote `
         -Target <app-verzeichnis>
```

### Python-Pakete

Das Build-System bringt normalerweise ein eigenes Python mit. Ohne das gebündelte Paket
müssen diese Module im verwendeten Interpreter liegen:
...
`grpcio-tools` wird gebraucht, weil nanopb sonst nach einer `protoc`-Binärdatei sucht; ist
es installiert, nutzt nanopb dessen mitgelieferten Compiler.

Eine Stelle im SDK muss noch entschärft werden: Die Erzeugung der VS-Code-Konfiguration ruft
`WhereIs("openocd")` und `WhereIs("clangd")` auf. Ohne das gebündelte Toolchain-Paket fehlen
beide, `WhereIs` liefert `None`, und der Build bricht ab, bevor überhaupt kompiliert wird.
In `scripts/fbt/util.py`:

```python
    @staticmethod
    def fix_path(path):
        if path is None:      # openocd/clangd fehlen ohne das Flipper-Bundle
            return ""
        return str(PurePosixPath(Path(path).as_posix()))
```

### Passt der Build zur Firmware?

Nach dem Bauen lässt sich prüfen, ob die App auf dem Zielgerät überhaupt laden wird. Eine
`.fap` ist eine ELF-Datei; ihre undefinierten Symbole müssen alle in der API-Tabelle der
Firmware stehen (`targets/f7/api_symbols.csv`):
...
print("fehlend:", sorted(undef - api))
```

Leere Liste heißt: die App startet. Genau hier fiel auf, dass ein Build gegen ein
**Fork-SDK** (Momentum) vier Icons als „Duplikate" weglässt, weil dieses Fork sie in der
Firmware mitliefert — auf der Original-Firmware fehlen sie dann und der Start scheitert mit
`MissingImports`.
...
├── ac_remote_app.c/.h              Einstiegspunkt, Lebenszyklus
├── ac_remote_app_i.h               Interne Struktur, Einstellungen
├── ac_remote_custom_event.h        Ereignis-Kodierung
├── scenes/
│   ├── ac_remote_scene.c/.h        Szenen-Verwaltung (generisch)
│   ├── ac_remote_scene_config.h    Szenenliste
│   └── ac_remote_scene_samsung.c   Die eigentliche Bedienoberfläche
├── views/
│   └── ac_remote_panel.c/.h        Button-Raster-Widget
├── lib/hvac_samsung/
│   └── hvac_samsung.c/.h           Protokoll: Frame bauen und senden
└── assets/                         Icons, werden zu C-Code kompiliert
```

Der Aufbau folgt dem üblichen Flipper-Muster: eine `ViewDispatcher` mit `SceneManager`,
darin ein `ViewStack` mit einem eigenen View-Widget.

---

## 6. Der Protokoll-Code

Dieser Teil ist unabhängig vom Flipper-UI und lässt sich einzeln testen.


### `lib/hvac_samsung/hvac_samsung.h`

Öffentliche Schnittstelle. Beachte, dass `hvac_samsung_set_swing()` **beide** Achsen nimmt — sie teilen sich ein Protokollfeld.
...
Frame-Aufbau, Prüfsummen und die Umsetzung in Timings.
...
`fap_private_libs` bindet den Protokollcode als eigene Bibliothek ein, `fap_icon_assets` kompiliert den `assets/`-Ordner zu `<appid>_icons.h`.
...
### Einstiegspunkt und Lebenszyklus
...
Hier steht die Einstellungsstruktur. Jedes neue Feature bekommt ein Feld.
...
Ereignistyp und Nutzlast werden in einen `uint32_t` gepackt, weil der ViewDispatcher nur einen Zahlenwert durchreicht.
...
Das ist die zentrale Datei: Layout, Zustandsverwaltung, Persistenz und das Senden.

Drei Dinge sind hier wichtig zu verstehen:

- **Bei ausgeschaltetem Gerät wird nichts gesendet.** Die Ereignisbehandlung ändert nur den
  Zustand und kehrt vorzeitig zurück. So lässt sich alles vorbereiten und dann mit einem
  einzigen Druck auf Power aktivieren.
- **Das Raster ist dünn besetzt.** `ac_remote_panel_reserve(2, 4)` legt 2×4 Plätze an, belegt
  sind sieben — die Navigation überspringt leere Plätze.
- **Beschriftungen speichern nur den Zeiger** auf den Text, keine Kopie. Deshalb dürfen dort
  ausschließlich String-Literale oder dauerhafte Puffer hinein.
...
Unverändert aus dem Ursprungsprojekt übernommen. Ein Raster aus Buttons mit Icon, Hover-Icon
und Rückruffunktion, dazu freie Beschriftungen und dekorative Icons.
...
Die Icons im `assets/`-Ordner werden beim Bauen zu C-Code übersetzt. Der Dateiname bestimmt
den Symbolnamen: `windfree_19x11.png` wird zu `I_windfree_19x11`.

Der Konverter wandelt das PNG nach 1 Bit um und **ignoriert dabei den Alphakanal**. Das führt
zu einer Konvention, die man kennen muss:

| Pixel im PNG | Ergebnis auf dem Display |
|---|---|
| deckendes Weiß `(255,255,255,255)` | **nicht** gezeichnet (Hintergrund) |
| transparentes Schwarz `(0,0,0,0)` | **gezeichnet** (schwarz) |

Die transparenten Pixel sind also die Tinte. Wer das übersieht, bekommt invertierte Icons.

Jeder Button hat zwei Zustände: das normale Icon und ein `_hover_`-Icon für die Auswahl,
üblicherweise die invertierte Fassung. Die beiden unteren Umschalter haben zusätzlich je eine
Ein-Variante mit Balken unter dem Symbol, also vier Dateien pro Button.

### Generator

Die vier neuen Icons wurden nicht gezeichnet, sondern erzeugt — so bleiben Rahmen und
Ausrichtung exakt identisch zu den vorhandenen 19×11-Buttons:


### `tools/mkicon.py`

Erzeugt `windfree_*` und `swing_h_*` in `assets/`.
...
### Textbreiten vorher ausrechnen

Das Display ist im Hochformat nur **64 Pixel breit**. Ob eine Überschrift hineinpasst, lässt
sich vorab bestimmen, statt es auszuprobieren: die u8g2-Schriftdaten liegen im Firmware-Baum
in `lib/u8g2/u8g2_fonts.c`, und die Summe der Glyph-Vorschübe ist genau das, was
`canvas_string_width()` auf dem Gerät liefert.

`FontPrimary` ist `helvB08`, `FontSecondary` ist `haxrcorp4089`.

Für die Überschrift ergab die Messung:

| Text | Schrift | Breite |
|---|---|---|
| `GNUSMAS` (Original) | FontPrimary | 55 px, ab x=6 → Ende bei 61 |
| `5RC - Gitzmod` | FontPrimary | 68 px — **passt nicht** |
| `5RC` | FontPrimary | 21 px |
| `Gitzmod` | FontSecondary | 35 px |

Daraus die finale Lösung: `5RC` fett ab x=1, `Gitzmod` dünn ab x=26, Ende bei x=61.


### `tools/fontwidth.py`

Liest die Schriftdaten aus dem Firmware-Baum und misst Textbreiten. Zwei Details, an denen ein naiver Parser scheitert: die Positionszeiger im Kopf stehen **big-endian**, und die Zeichendaten enthalten literale `;` und `"`, weshalb sich das C-Array nicht am Semikolon abgrenzen lässt.
...
Ergebnis: `build\f7-firmware-D\.extapps\samsung_ac_remote.fap`

Am Ende steht `APPCHK` mit `Target: 7, API: 87.1` — das ist die Zusicherung, dass Build und
Firmware zusammenpassen.
...
Das kopiert die Datei und startet die App direkt. Alternativ die `.fap` einfach per qFlipper
nach `apps/Infrared/` auf die SD-Karte ziehen.

> **Der COM-Port muss frei sein.** Alles, was eine serielle Verbindung zum Flipper offen hält,
> blockiert das: qFlipper, ein Terminal — und, weniger offensichtlich, ein Browser-Tab mit
> einem Flipper-Webtool. Die Web-Serial-API hält den Port und **greift ihn nach jedem
> Einstecken automatisch wieder ab**.

---

## 11. Screenshots erzeugen

Die Firmware kann ihren Bildspeicher über RPC ausgeben (`Gui.StartScreenStream`). Ein
fertiges Werkzeug dafür gibt es nicht, es ist aber schnell gebaut.

**Ablauf:** Serielle Verbindung öffnen, `start_rpc_session` senden, danach
längenpräfigierte Protobuf-Nachrichten austauschen. Die Protokolldefinitionen liegen im
Firmware-Baum unter `assets/protobuf/`:

```bash
cd <fw>/assets/protobuf
python -m grpc_tools.protoc -I. --python_out=<ziel> *.proto
```

Ein Bild ist 1024 Byte im u8g2-Seitenformat: Byte `x + (y / 8) * 128`, Bit `y % 8`. Läuft die
App im Hochformat, meldet das Frame `orientation = 2` und muss um 90° gedreht werden.

**Zwei Stolpersteine:**

- Der CLI-Befehl muss mit **einem einzelnen `\r`** abgeschlossen werden. Ein zusätzliches
  `\n` landet als `0x0A` im RPC-Datenstrom, wird dort als Längenangabe gelesen und bringt
  alles Folgende aus dem Tritt.
- **Tastendrücke über RPC einzuspielen ist unzuverlässig.** Auf dieser Firmware kamen die
  Ereignisse teils gar nicht an. Deterministische Screenshots entstehen stattdessen so:
  Einstellungsdatei schreiben, App neu starten, Bild greifen. Die App lädt ihre Einstellungen
  beim Start, damit ist jeder Zustand exakt reproduzierbar.


### `tools/flipshot.py`

RPC-Client und PNG-Ausgabe.
...
Fährt alle dokumentierten Zustände über die Einstellungsdatei an.
...
Gesammelt aus dem Bau dieser App — alles Dinge, die Zeit gekostet haben.

**Protokoll**

- Power steht **zweimal** im Frame (Byte 6 und 13).
- Vertikaler und horizontaler Swing teilen sich **ein** 3-Bit-Feld. Eine Achse einzeln zu
  schreiben löscht die andere.
- WindFree braucht Lüfter auf Auto und V-Swing aus, sonst ignoriert es das Gerät.
- Die Prüfsumme lässt die Prüfsummen-Nibbles selbst aus und wird auf zwei Nibbles in
  **verschiedenen** Bytes verteilt.
- Der Bereich 16–30 °C ist der des **Protokolls**. Reale Geräte sind oft enger — diese Anlage
  kühlt erst ab 18 °C und verwirft Frames darunter kommentarlos. Das sieht dann aus, als
  liefe nur der Ventilator.

**Assets**

- Transparente Pixel sind die Tinte, nicht die weißen.
- Ein Build gegen ein Fork-SDK kann Icons als „Duplikate" weglassen, wenn das Fork sie in der
  Firmware mitbringt. Auf Original-Firmware fehlen sie dann → `MissingImports`. Immer gegen
  die API-Tabelle der Firmware prüfen, die tatsächlich auf dem Gerät läuft.

**Oberfläche**

- Beschriftungen speichern nur den **Zeiger** auf den Text. Kein Stack-Puffer.
- Bei hochkant gedrehter Ansicht **dreht die Firmware das Steuerkreuz mit**: Was das Widget
  als „runter" sieht, ist physisch „rechts" (`view_port_input_mapping` in `view_port.c`).
- 64 px Breite sind schnell voll. Textbreiten vorher aus den Schriftdaten rechnen.

**Werkzeuge**

- Der COM-Port wird gern von etwas anderem gehalten — qFlipper, Terminal, oder ein
  Browser-Tab mit Web-Serial.
- Beim Aufräumen in Skripten aufpassen: Ein Recorder, der beim Start sein Ausgabeverzeichnis
  leert, löscht bei einem fehlgeschlagenen zweiten Start die Aufnahmen des noch laufenden
  ersten.
...
GPL, wie das Ursprungsprojekt. Siehe `LICENSE` im App-Verzeichnis.
